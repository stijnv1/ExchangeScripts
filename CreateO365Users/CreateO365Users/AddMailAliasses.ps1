...
Function AddEmailAddressRemoteMailbox
{
    ...
    Try
    {
        ...
        foreach ($mailaddress in $Aliasses)
        {
            ...
            {
                WriteToLog -LogPath $LogDirPath -TextValue "Added e-mail address $mailaddress successfully" -WriteError $true
            }
            else
            {
...
Try
{
    ...
    Invoke-Command -ComputerName $AzureADSyncServerName -ScriptBlock {& cmd.exe /c "D:\Program Files\Microsoft Azure AD Sync\Bin\DirectorySyncClientCmd.exe"}
}
